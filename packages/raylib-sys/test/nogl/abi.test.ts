...
import { dlopen, ptr, CString, type FFIFunction } from "bun:ffi";
...
test("linked raylib matches the pinned version", () => {
  expect(new CString(raylib.probe_raylib_version()).toString()).toBe(RAYLIB_VERSION);
});
